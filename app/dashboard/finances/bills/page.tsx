...
export default function BillsPage() {
  ...
  const [selectedBill, setSelectedBill] = useState<InvoiceItem | null>(null);
  ...
  const bills = (data?.invoices ?? []).filter((i) => i.type === 'in');
  let filtered = ageBucket === 'all' ? bills : bills.filter((i) => (i.ageBucket || '0-30') === ageBucket);

  const totalOutstanding = bills.reduce((s, i) => s + i.amount, 0);
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] bg-rta-bg-light">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-8 lg:py-10">
        ...
        <h1 className="text-h3 font-bold text-rta-blue">Bills</h1>
        <p className="text-body-sm text-rta-text-secondary mt-1">
          Xero supplier bills (ACCPAY) with due date and ageing
          {data?.source === 'xero' && (
            <span className="ml-2 text-rta-blue font-medium">· Live from Xero</span>
          )}
        </p>

        <Card className="relative border-rta-border bg-white shadow-card overflow-hidden group hover:shadow-card-hover transition-all duration-300 mt-6 mb-6">
          ...
          <CardContent className="relative pt-6">
            <p className="text-body-sm text-rta-text-secondary font-medium">Total outstanding (bills to pay)</p>
            <p className="text-2xl font-bold text-rta-text tabular-nums mt-1">{formatAmount(totalOutstanding)}</p>
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-6">
            {loading ? (
              <div className="flex items-center gap-2 py-12 justify-center text-rta-text-secondary">
                ...
                <span className="text-body-sm">Loading bills…</span>
              </div>
            ) : (
              <>
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="bg-rta-bg-light border-b border-rta-border">
                        <th className="px-4 py-3 text-body-sm font-semibold text-rta-text">Supplier</th>
                ...
                {filtered.length === 0 && (
                  <p className="py-8 text-center text-body-sm text-rta-text-secondary">No bills match the filters.</p>
                )}
              </>
            )}
          </CardContent>
              ...
              <div className="relative bg-gradient-to-br from-rta-bg-light via-white to-rta-bg-blue/30 pt-6 pb-6 px-6 border-b border-rta-border">
                ...
                <div className="relative flex items-start justify-between gap-4">
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-2">
                      <Badge variant="outline" className="border-rta-border text-rta-text">Bill (to pay)</Badge>
                ...
                <div className="relative mt-4 pt-4 border-t border-rta-border/60">
                  ...
                  <p className="text-2xl font-bold text-rta-text tabular-nums">
                    {formatAmount(selectedBill.amount, selectedBill.currency)}
                  </p>
                </div>
              </div>
              <div className="overflow-y-auto flex-1 p-6 space-y-4">
                <div className="rounded-lg bg-rta-bg-light/80 border border-rta-border/60 p-4">
                  ...
                  <dl className="grid grid-cols-1 gap-2 text-body-sm">
                    <div>
                      <dt className="text-rta-text-secondary font-medium">Supplier</dt>
                      <dd className="text-rta-text font-medium">{selectedBill.entity}</dd>
                    </div>
                    <div>
                      <dt className="text-rta-text-secondary font-medium">Due date</dt>
                      <dd className="text-rta-text">{formatDate(selectedBill.dueDate)}</dd>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <dd className="text-rta-text">{(selectedBill.status || '').replace(/_/g, ' ')}</dd>
                    </div>
                  </dl>
                </div>
              </div>
